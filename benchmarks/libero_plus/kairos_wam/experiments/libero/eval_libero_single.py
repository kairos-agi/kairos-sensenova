...
from kairoswam.datasets.lerobot.processors.kairos_wam_processor import KairosWAMProcessor
from kairoswam.datasets.lerobot.utils.normalizer import load_dataset_stats_from_json
from kairoswam.utils.pytorch_utils import set_global_seed
...
def _normalize_mujoco_egl_device_id() -> None:
    """Use the process-local EGL ordinal after robosuite's import-time CUDA check."""
    mode = os.environ.get("LIBERO_EGL_DEVICE_ID_MODE", "local").strip().lower()
    if mode in {"physical", "global", "as_is", "none"}:
        return
    ...
    visible_ids = [part.strip() for part in cuda_visible.split(",") if part.strip()]
    if egl_device is None:
        os.environ["MUJOCO_EGL_DEVICE_ID"] = "0"
    elif egl_device in visible_ids:
        os.environ["MUJOCO_EGL_DEVICE_ID"] = str(visible_ids.index(egl_device))
...
def _is_external_model(model: torch.nn.Module) -> bool:
    return bool(getattr(model, "is_external_model", False))
...
def _denormalize_action(action: torch.Tensor, processor: KairosWAMProcessor) -> np.ndarray:
    if action.ndim == 2:
...
def _get_max_steps(task_suite_name: str) -> int:
    suite_steps = {
        ...
        "libero_10": 700,
        "libero_90": 700,
        # LIBERO-Plus perturbation benchmark.
        # Empirically similar to libero_10 in many eval scripts.
        "libero_mix": 520,
    }
...
def _sanitize_category_value(category_value: str) -> str:
    # Match the common convention used in LIBERO-Plus scripts.
...
def eval_single_process(cfg: DictConfig):
    ...
    if cfg.ckpt is None:
        if not _is_external_model(model):
            raise ValueError("cfg.ckpt must not be None for non-external models.")
        logging.info("External model detected; skip checkpoint loading because cfg.ckpt is None.")
    else:
    ...
    processor: KairosWAMProcessor = instantiate(cfg.data.train.processor).eval()
    ...
    suite_dir = local_log_dir / cfg.EVALUATION.task_suite_name
    if category_value is not None and str(category_value).strip() != "":
        suite_dir = suite_dir / _sanitize_category_value(str(category_value))
    elif str(cfg.EVALUATION.task_suite_name) == "libero_mix":
        raise ValueError("EVALUATION.category_value must be set for task_suite_name=libero_mix.")
    ...
    initial_states = list(task_suite.get_task_init_states(cfg.EVALUATION.task_id))
    if not initial_states:
        raise ValueError(f"No initial states found for task_id={cfg.EVALUATION.task_id}")
