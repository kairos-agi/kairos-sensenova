...
def sag_attention_with_window(
    ...
    **kwargs: Any,
    ...
    if(window_size_left == -1 and window_size_right == -1): #都是-1，走全流程注意力
        return sageattn_qk_int8_pv_fp16_cuda(q,k,v,is_causal=is_causal,qk_quant_gran=qk_quant_gran, pv_accum_dtype=pv_accum_dtype,smooth_v=smooth_v)
    ...
    if pv_accum_dtype == 'fp16':
        if smooth_v:
            smoothed_v, vm = sub_mean(v, tensor_layout=tensor_layout)
            smoothed_v, vm = sub_mean(v, tensor_layout=tensor_layout)
            lse = sm80_compile.qk_int8_sv_f16_accum_f16_fuse_v_mean_attn_window_size(q_int8, k_int8, smoothed_v, o, window_size_left,window_size_right, q_scale, k_scale, vm, _tensor_layout, _is_caual, _qk_quant_gran, sm_scale, _return_lse)
