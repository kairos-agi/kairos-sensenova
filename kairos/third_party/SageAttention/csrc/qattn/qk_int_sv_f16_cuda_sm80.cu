...
template<uint32_t CTA_Q, uint32_t CTA_K, uint32_t WARP_Q, uint32_t WARP_K, uint32_t head_dim, DataType DTypeQK, QuantGranularity Q_GRAN, QuantGranularity K_GRAN,
        typename DTypeSVAccum = float, bool use_inst_buffer = false, typename DTypeOut = half, ComputeUnit DenominatorAccumUnit, MaskMode mask_mode = MaskMode::kNone, bool return_lse = false, bool fuse_v_mean=false>
__global__ void qk_int_sv_f16_attn_kernel_window_size(int8_t *__restrict__ Q, int8_t *__restrict__ K, half *__restrict__ V, DTypeOut *__restrict__ O, float *__restrict__ Lse,
                      float *__restrict__ Q_scale, float *__restrict__ K_scale, DTypeOut *__restrict__ V_mean,
                      const uint32_t qo_len, const uint32_t kv_len, const uint32_t num_kv_groups,
                      const uint32_t stride_bz_q, const uint32_t stride_seq_q, const uint32_t stride_h_q,
                      const uint32_t stride_bz_k, const uint32_t stride_seq_k, const uint32_t stride_h_k,
                      const uint32_t stride_bz_v, const uint32_t stride_seq_v, const uint32_t stride_h_v,
                      const uint32_t stride_bz_o, const uint32_t stride_seq_o, const uint32_t stride_h_o,
                      float sm_scale, const uint32_t window_size_left, const uint32_t window_size_right)
{
  // compile time check
  static_assert(DTypeQK == DataType::kInt8 || DTypeQK == DataType::kInt4, "DTypeQK must be int8 or int4");
  static_assert(Q_GRAN == QuantGranularity::kPerBlock || Q_GRAN == QuantGranularity::kPerWarp || Q_GRAN == QuantGranularity::kPerThread, "Q_GRAN must be kPerBlock, kPerWarp or kPerThread");
...
#pragma unroll
  ...
  //   printf("Q_idx_lane_base start:%d, K_idx_lane_base start:%d\n", Q_idx_lane_base, K_idx_lane_base);
  // }
  const uint32_t num_iterations = div_ceil(
      mask_mode == MaskMode::kCausal
          ? min(kv_len, (bx + 1) * CTA_Q)
...
#pragma unroll
  ...
  V_load_idx_lane_base += CTA_K;
  // Q block range
  const int q_block_start = int(bx * CTA_Q);
  const int q_block_end   = min(q_block_start + int(CTA_Q), int(qo_len));

  int32_t kv_start = max(0, static_cast<int32_t>(q_block_start + kv_len - qo_len) - static_cast<int32_t>(window_size_left));
  int32_t kv_end   = min(static_cast<int32_t>(kv_len), static_cast<int32_t>(q_block_end + kv_len - qo_len) + static_cast<int32_t>(window_size_right));
  uint32_t iter_min = kv_start / CTA_K; // k的iter起始位置
  uint32_t iter_max = div_ceil(kv_end, CTA_K); // k的iter结束位置
  uint32_t iter_begin = max(1, iter_min);
  uint32_t iter_end   = min(iter_max-1, num_iterations-1);

  #pragma unroll
  for (uint32_t iter = 1; iter < iter_begin; iter++)
  {
      // 只推进语义 index
      K_idx_lane_base += CTA_K;

      // 2️⃣ global pointer 推进（关键）
      K_lane_base_ptr += CTA_K * stride_seq_k;
      V_lane_base_ptr += CTA_K * stride_seq_v;

      // 3️⃣ load idx 推进（给 predicate / 下一轮用）
      K_load_idx_lane_base += CTA_K;
      V_load_idx_lane_base += CTA_K;
      dequant_scale = q_scale * K_scale[k_scale_idx + iter * k_scale_advance_offset];
      sm_scale = original_sm_scale * dequant_scale;
...
#pragma unroll
  for (uint32_t iter = iter_begin ; iter < iter_end; iter++)
  {
    // ensure K is ready
    cp_async::wait_group<1>();
    __syncthreads();
...
#pragma unroll
    ...
    apply_sag_mask<num_tiles_q, num_tiles_k>(Q_idx_lane_base, K_idx_lane_base, RS_f32, qo_len, kv_len, window_size_left, window_size_right);
    // do not apply causal mask and out of bound mask for these iterations
    K_idx_lane_base += CTA_K;
...
torch::Tensor qk_int8_sv_f16_accum_f16_fuse_v_mean_attn_window_size(torch::Tensor query,
                    torch::Tensor key,
                    torch::Tensor value,
                    torch::Tensor output,
      ...
      DISPATCH_QK_QUANT_GRAN(qk_quant_gran, QK_QUANT_GRAN, {
        DISPATCH_RETURN_LSE(return_lse, RETURN_LSE, {
          DISPATCH_PYTORCH_DTYPE_TO_CTYPE_FP16(output_dtype, DTypeOut, {
            ...
            auto kernel_func = qk_int_sv_f16_attn_kernel_window_size<CTA_Q, CTA_K, WARP_Q, WARP_K, HEAD_DIM, DataType::kInt8, static_cast<QuantGranularity>(QK_QUANT_GRAN), static_cast<QuantGranularity>(QK_QUANT_GRAN), half, false, DTypeOut, ComputeUnit::kTensorCore, 
                                                          mask_mode, RETURN_LSE, true>;
