...
template <uint32_t num_tiles_q, uint32_t num_tiles_k, typename DTypeQKAccum>
__device__ __forceinline__ void apply_sag_mask(const uint32_t &Q_idx_lane_base, const uint32_t &K_idx_lane_base, DTypeQKAccum RS[][num_tiles_k][8],
    uint32_t qo_len, uint32_t kv_len, int window_left, int window_right) {
#pragma unroll
  for (uint32_t fq = 0; fq < num_tiles_q; fq++)
  {
#pragma unroll
...
#pragma unroll
      for (uint32_t k = 0; k < 8; k++)
      {
              const uint32_t kv_idx = K_idx_lane_base + fk * 16 + 8 * (k / 4) + k % 2;
              const uint32_t q_idx = Q_idx_lane_base + fq * 16 + 8 * ((k % 4) / 2);
              const int q_i  = int(q_idx);
              const int kv_l = int(kv_len);
              const int qo_l = int(qo_len);

              const int center = q_i + kv_l - qo_l;

              const int kv_idx_limit_left  = max(0, center - window_left);
              const int kv_idx_limit_right = min(kv_l, center + 1 + window_right);
              bool out_of_boundary = false;
              if(kv_idx < kv_idx_limit_left || kv_idx >= kv_idx_limit_right) {
