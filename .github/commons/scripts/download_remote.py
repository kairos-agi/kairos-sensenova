...
def main():
    ...
    # 下载gt video， 为video_checker.py 做准备
    gt_remote_path = os.path.join(f"s3://{ak}:{sk}@{remote_endpoint}", f"kairos-ci-gt/{job_name}/output.mp4")
    gt_local_path = os.path.join(f".github/gt_resource/{job_name}/output.mp4")
    gt_local_dir_path = os.path.dirname(gt_local_path)
    run_cmd(f"mkdir -p {gt_local_dir_path} || true && ./ads-cli -q cp {gt_remote_path} {gt_local_path}")
